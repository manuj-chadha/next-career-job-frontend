...
const Navbar = () => {
  ...
  const navItemClasses = (path) =>
    `transition-all duration-200 hover:text-[#6A38C2] hover:scale-105 ${
      isActive(path) ? 'text-[#6A38C2] font-semibold' : 'text-gray-700'
    }`;

  const navItemMobileClasses = (path) =>
    `transition-all duration-200 hover:text-[#6A38C2] hover:scale-105 shadow-sm rounded-b-lg px-4 py-3 ${
      isActive(path) ? 'bg-[#f8f4ff] text-[#6A38C2] font-medium' : 'text-gray-700'
    }`;
      ...
      <div className='flex items-center justify-between mx-auto max-w-7xl h-16 px-3'>
        ...
        <ul className='hidden md:flex font-medium text-md items-center gap-6'>
          {user && user.role.toLowerCase() === 'recruiter' ? (
            <>
              <li><Link to="/admin/companies" className={navItemClasses("/admin/companies")}>Companies</Link></li>
              <li><Link to="/admin/jobs" className={navItemClasses("/admin/jobs")}>Jobs</Link></li>
            </>
          ) : (
            <>
              <li><Link to="/" className={navItemClasses("/")}>Home</Link></li>
              <li><Link to="/jobs" className={navItemClasses("/jobs")}>Jobs</Link></li>
              <li><Link to="/browse" className={navItemClasses("/browse")}>Browse</Link></li>
              <li><Link to="/career-chat-ai" className={navItemClasses("/career-chat-ai")}>Career AI</Link></li>
            </>
          )}
        </ul>
        ...
        <div className='hidden md:flex items-center gap-4'>
          {!user ? (
            <>
              <Link to="/login">
                <Button variant="outline" className='transition hover:scale-105'>Login</Button>
              </Link>
              <Link to="/signup">
                <Button className="bg-[#6A38C2] hover:bg-[#5b30a6] transition hover:scale-105">Signup</Button>
              </Link>
            </>
          ) : (
            <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen}>
              ...
              <PopoverContent className="w-80 max-w-xs sm:max-w-sm overflow-x-hidden mx-2 border-none shadow-md">
                      ...
                      <div className='flex items-center gap-2'>
                        ...
                        <Link to="/profile">
                          <Button
                            ...
                            className='text-sm border-none outline-none shadow-none focus-visible:ring-0'
                          >
                            View Profile
                          </Button>
                        </Link>
        ...
        <div className="md:hidden flex items-center gap-3">
          ...
          {user && (
            <Popover>
              ...
              <PopoverContent className="w-80 max-w-xs sm:max-w-sm overflow-x-hidden border-none shadow-md">
                <div className='flex flex-col gap-3'>
                  <div className='flex items-center gap-3'>
                    <Avatar>
                      <AvatarImage src={user?.profile?.profilePhoto || defaultPic} />
          ...
          >
            ...
            {!user && (
              <div className='flex flex-col gap-2 mt-4'>
                <Link to="/login">
                  <Button variant="outline" className="w-full transition hover:scale-105">Login</Button>
                </Link>
                <Link to="/signup">
                  <Button className="bg-[#6A38C2] hover:bg-[#5b30a6] w-full transition hover:scale-105">Signup</Button>
                </Link>
              </div>
            )}
          </motion.div>
